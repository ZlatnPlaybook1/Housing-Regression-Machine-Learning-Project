"""
Preprocessing Scripts for Housing Regression MLE

- Reads train/eval/holout CSVs Data/raw/.
- Clean and normalize city names
- Map cities to metros and merges lat/lng.
- Drop duplicates abd extreme outliers.
...
def normalize_city(s: str) -> str:
    """Lowercase, strip, unify dashes, Save for NA."""
    if pd.isna(s):
        return 
    s = str(s).strip().lower()
...
def clean_and_merge(df: pd.DataFrame, metros_path: str | None = "Data/raws/usmetros.csv") -> pd.DataFrame:
    """
    ...
    # if lat/lan already present , skip merge
...
def drop_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    before = df.shape[0]
    df = df.drop_duplicates(subset=[col for col in df.columns if col not in ["date", "year"]], keep=False)
    after = df.shape[0]
