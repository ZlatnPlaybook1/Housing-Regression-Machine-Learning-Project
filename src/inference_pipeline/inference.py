...
DEFAULT_OUTPUT = PROJECT_ROOT / "prediction.csv"

print("Inference using project root:", PROJECT_ROOT)

# Load training feature columns (strict schema from training dataset)
if Path(TRAIN_FE_PATH).exists():
    _train_cols = pd.read_csv(TRAIN_FE_PATH, nrows=1)
    TRAIN_FEATURE_COLIMNS = [c for c in  _train_cols.columns if c != "price"]
else:
    TRAIN_FEATURE_COLIMNS = None
...
def predict(
    ...
    # Aliggn coulmns with training schema
    if TRAIN_FEATURE_COLIMNS is not None:
        df = df.reindex(columns=TRAIN_FEATURE_COLIMNS , fill_value=0)
